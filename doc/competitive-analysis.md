...
**Relevance to OpenMontage:** Royal validated the model of fractional creative revenue ownership via NFTs. Its challenges — token liquidity, marketplace sustainability, regulatory complexity around securities — are instructive. OpenMontage's revenue model (75/10/5/10 split via smart contracts) should study Royal's experience with automated royalty distribution and SEC considerations.
...
**Relevance to OpenMontage:** Coordinape's peer allocation model could complement OpenMontage's Shot Market curation system — potentially for evaluating Standard Library contributions or other non-video work that doesn't fit the Shot-based pull request model.
...
|---|---|---|---|---|---|---|---|
| **OpenMontage** | Crowdsourced AI Filmmaking | Shot-based PR contributions, Shot Market curation | 75% creators / 10% Film NFT holder / 5% Standard Library / 10% backers (via Shot Market competition + streaming) | Standard Library (LoRAs, style guides, ControlNets) | Shot Market (economic backing, quadratic weighting) for film-level; $MONTAGE token-weighted for platform governance | Unified pipeline from script to screen with visual consistency | Pre-launch; unproven at scale |
...
**Gap 3: No granular, per-contribution revenue sharing for video.** Story Protocol automates IP royalties but at the asset level. Royal.io distributes music royalties. But no platform distributes film revenue proportionally to screen time contribution at the Shot level, tracked through NFT provenance. OpenMontage's Shot Market backing mechanism adds a quality selection layer on top of this, where backers stake real capital to identify the best clips — transforming passive curation into economic investment.

**Gap 4: Git-style version control for creative video does not exist.** The pull request model — submit a Shot, community reviews and backs clips via Shot Markets, best version merges to main branch — applies proven software development workflows to film production, with economic backing via Shot Markets (quadratic weighting) replacing token voting as the quality selection mechanism. This is conceptually powerful and entirely unoccupied.

**Gap 5: Discord-first DAO filmmaking is unexplored.** While Dework and Coordinape provide DAO task management through Discord, no platform has built a complete filmmaking workflow — from Shot assignment through submission, review, economic backing via Shot Markets, and merging — natively within Discord.
...
- **Zora's creator economics:** The 50% creator fee share drove real adoption. OpenMontage's 75% creator share remains generous, and the Shot Market backing mechanism additionally rewards quality curators with 10% of streaming revenue — emphasize this dual incentive in positioning.
...
- **Glass.xyz's failure:** “No sustainable demand for video NFTs” in a pure collectible model. OpenMontage must ensure Clip NFTs have functional utility (revenue rights, economic backing positions) beyond speculation.
